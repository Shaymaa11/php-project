...
$resobj = $stm->execute([$userid, $prodids]);
...
if (isset($_GET['add'])) {
    ...
    while ($row = $stm->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($amount < $rowprods) {
            ...
            $resobj = $stmt->execute([$amount, $rowprodf, $userid, $prodids]);
        } else {
...
if (isset($_GET['dele'])) {
    ...
    while ($row = $stm->fetch(PDO::FETCH_ASSOC)) {
        if ($row['amount'] > 1) {
            ...
            $stmt = $db->prepare($update_query);
            $resobj = $stmt->execute([$amount, $rowp, $userid, $prodids]);
        } else {
...
if (isset($_GET['remov'])) {
    ...
    while ($row = $stm->fetch(PDO::FETCH_ASSOC)) {
        ...
        $stmt = $db->prepare($update_query);
        $resobj = $stmt->execute([$canceldamount, $userid, $prodids]);
    }
}

$select_query = "select * from users where id=? ";
$stmt = $db->prepare($select_query);
$resobj = $stmt->execute([$userid]);
    ...
    <div class="humberger__menu__wrapper">
        ...
        <nav class="humberger__menu__nav mobile-menu">
            <ul>
                ...
                <li><a href="./shoping-cart.php?id=<?php echo $userid; ?>">Shoping Cart</a>
                </li>
                <li><a href="../admin/allproduct.php">Products</a></li>
                <li><a href="../controls/allUsers.php">Users</a></li>
                <li><a href="../admin/checks.php">Checks</a></li>
                <li><a href="../admin_cafeterai/manualOrder.php">Manual Order</a></li>
...
<td>
<a href='shoping-cart.php?add=true&id={$row['id_user']}&prodids={$row['product_id']}'>
<input type='button' value='+'></a></td>
<td> <a href='shoping-cart.php?dele=true&id={$row['id_user']}&prodids={$row['product_id']}'>
<input type='button' value='-'></a></td>
 <td> <a href=' shoping-cart.php?remov=true&id={$row['id_user']}&prodids={$row['product_id']}'>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="shoping__cart__btns">
                        <a href="./index.php?selectedid=<?php echo $userid; ?>" class="primary-btn cart-btn">CONTINUE SHOPPING</a>
